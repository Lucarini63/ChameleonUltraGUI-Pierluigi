...
String chameleonTagToString(TagType tag, AppLocalizations localizations) {
  ...
  } else if (tag == TagType.mifareDesfire) {
    ...
    if (tag.data.isEmpty) {
      ...
      await showDialog<void>(
        ...
        builder: (context) => AlertDialog(
          title: Text(
              italian ? 'Nessun dump di memoria' : 'No memory dump available'),
          content: Text(italian
              ? 'Questa credenziale LF trasmette un identificativo ma non contiene pagine di memoria da esportare. Usa il formato JSON per conservare tutti i dati letti.'
              : 'This LF credential transmits an identifier but has no memory pages to export. Use JSON to preserve all decoded data.'),
          actions: [
            TextButton(
...
Future<(HFCardInfo, MifareClassicInfo, MifareUltralightInfo)> readHFInfo(
  ...
  try {
    ...
    final isIso14443_4 = (card.sak & 0x20) != 0 && card.ats.isNotEmpty;
    if (isIso14443_4) {
      hfInfo.desfireInfo = await identifyMifareDesfire(
        appState.communicator!,
      );
      if (hfInfo.desfireInfo!.confirmed) {
        type = TagType.mifareDesfire;
      }
    } else if (!await appState.communicator!.detectMf1Support()) {
      (type, mfuInfo) =
    ...
    if (hfInfo.desfireInfo?.confirmed ?? false) {
      hfInfo.tech = hfInfo.desfireInfo!.displayName;
    } else if (isIso14443_4) {
      hfInfo.tech = "ISO/IEC 14443-4";
    } else {
