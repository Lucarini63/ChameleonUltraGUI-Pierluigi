...
enum ChameleonCommand {
  ...
  // ISO/IEC 14443-4 reader commands (available on supported firmware)
...
enum TagType {
  ...
  ultralight11(1105),
  ultralight21(1106),
  // Detection-only: DESFire emulation/writing is not exposed by this GUI.
  mifareDesfire(1300);
...
class HIDCard extends LFCard {
  ...
  String toViewableString() {
    String out =
        '$credentialNumber (${bytesToHexSpace(uid)}, ${getNameForHIDProxType(hidType)}';
  ...
  int get credentialNumber {
    final high = uid[0];
    final low = (uid[1] << 24) | (uid[2] << 16) | (uid[3] << 8) | uid[4];
    return (high << 32) | low;
  }
