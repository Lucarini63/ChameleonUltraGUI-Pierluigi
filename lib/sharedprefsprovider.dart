    ...
    for (var key in input.split("\n")) {
      ...
      if (allowedKeySizes.contains(key.length) &&
        ...
        if (seenKeys.add(normalizedKey)) {
          keys.add(hexToBytes(normalizedKey));
        }
      }
    }
...
class CardSave {
  ...
  String toJson() {
    ...
    if (chameleonTagToFrequency(tag) == TagFrequency.lf) {
      final card = getLFCardFromUID(tag, uid);
      if (card is HIDCard) {
        output['lf'] = {
          'technology': 'HID Prox',
          'format': getNameForHIDProxType(card.hidType),
          'formatCode': card.hidType,
          'facilityCode': card.facilityCode,
          'credentialNumber': card.credentialNumber,
          'identifierHex': bytesToHexSpace(card.uid),
          'issueLevel': card.issueLevel,
          'oem': card.oem,
          'encodedData': card.toString(),
        };
      } else {
        output['lf'] = {
          'technology': tag.name,
...
class SharedPreferencesProvider extends ChangeNotifier {
  ...
  bool isAutomaticConnectionEnabled() {
    return _sharedPreferences.getBool('automatic_connection') ?? false;
  }

  void setAutomaticConnectionEnabled(bool value) {
    _sharedPreferences.setBool('automatic_connection', value);
  }
  ...
  List<Dictionary> getDictionaries({int keyLength = 0}) {
    ...
    output.sort((first, second) {
      final keyCountComparison =
          first.keys.length.compareTo(second.keys.length);
      if (keyCountComparison != 0) return keyCountComparison;
  ...
  bool areDefaultDictionariesDownloaded() {
    return _sharedPreferences.getBool('default_dictionaries_downloaded') ??
        false;
  }

  void setDefaultDictionariesDownloaded(bool value) {
    _sharedPreferences.setBool('default_dictionaries_downloaded', value);
  }

  String? getSelectedMifareClassicDictionaryId() {
    return _sharedPreferences.getString('selected_mifare_classic_dictionary');
  }

  void setSelectedMifareClassicDictionaryId(String? id) {
    if (id == null || id.isEmpty) {
      _sharedPreferences.remove('selected_mifare_classic_dictionary');
    } else {
      _sharedPreferences.setString('selected_mifare_classic_dictionary', id);
    }
  }

  // MIFARE Classic key-check method:
  //   0 = single-block (mf1CheckKeysOnBlock, the proven default)
  //   1 = check-keys-of-sectors (mf1CheckKeysOfSectors, 2012), falls back to 0
  //       automatically if the device errors
  //   2 = compare: run both on the same card and log a byte-for-byte diff, then
  //       apply the single-block result
  int getMifareClassicKeyCheckMethod() {
    return _sharedPreferences.getInt('mfc_key_check_method') ?? 0;
  }

  void setMifareClassicKeyCheckMethod(int value) {
    _sharedPreferences.setInt('mfc_key_check_method', value);
  }
