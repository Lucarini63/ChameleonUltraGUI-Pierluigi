...
class ChameleonCommunicator {
  ...
  Future<Uint8List> sniffLF({int timeoutMs = 2000}) async {
    final duration = timeoutMs.clamp(1, 10000);
    final response = await sendCmd(
      ChameleonCommand.lfSniff,
      data: Uint8List.fromList([duration >> 8, duration & 0xff]),
      timeout: Duration(milliseconds: duration + 5000),
    );
    if (response == null || response.status != 0x40 || response.data.isEmpty) {
      throw Exception(
          'LF capture failed (status ${response?.status ?? 'timeout'})');
    }
    return response.data;
  }

  Future<Uint8List> sniffHF({int timeoutMs = 5000}) async {
    final duration = timeoutMs.clamp(1, 30000);
    ...
    if (response == null || response.status != 0 || response.data.isEmpty) {
      throw Exception(
          'HF capture failed (status ${response?.status ?? 'timeout'})');
    }
    return response.data;
  }
  ...
  /// Sends an ISO/IEC 14443-4 APDU using firmware-side T=CL handling.
  ///
  /// This command is intentionally kept separate from [send14ARaw]: the
  /// firmware performs selection, RATS, PCB framing and CRC handling.
  Future<Uint8List> send14A4Apdu(Uint8List apdu) async {
    final response = await sendCmd(
      ChameleonCommand.hf14A4ReaderApdu,
      data: apdu,
      timeout: const Duration(seconds: 3),
    );
    if (response == null || response.status != 0x00) {
      throw Exception(
          'ISO 14443-4 APDU failed (status ${response?.status ?? 'timeout'})');
    }
    return response.data;
  }
