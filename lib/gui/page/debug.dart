        ...
        body: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(localizations.mfc_key_check_method,
                  textScaler: const TextScaler.linear(1.5)),
              const SizedBox(height: 5),
              Text(localizations.mfc_key_check_method_description,
                  textAlign: TextAlign.center,
                  textScaler: const TextScaler.linear(0.9)),
              const SizedBox(height: 8),
              DropdownButton<int>(
                value: appState.sharedPreferencesProvider
                    .getMifareClassicKeyCheckMethod(),
                items: [
                  DropdownMenuItem<int>(
                    value: 0,
                    child: Text(localizations.mfc_key_check_single_block),
                  ),
                  DropdownMenuItem<int>(
                    value: 1,
                    child: Text(localizations.mfc_key_check_sectors),
                  ),
                  DropdownMenuItem<int>(
                    value: 2,
                    child: Text(localizations.mfc_key_check_compare),
                  ),
                ],
                onChanged: (value) {
                  if (value != null) {
                    appState.sharedPreferencesProvider
                        .setMifareClassicKeyCheckMethod(value);
                    appState.changesMade();
                  }
                },
              ),
              const SizedBox(height: 20),
              Text(localizations.log_file,
                  textScaler: const TextScaler.linear(1.5)),
              const SizedBox(height: 5),
              Text(localizations.log_file_description,
                  textAlign: TextAlign.center,
                  textScaler: const TextScaler.linear(0.9)),
              const SizedBox(height: 5),
              Padding(
                ...
                child: SelectableText(
                  FileLog.path ?? localizations.log_file_unavailable,
                  textAlign: TextAlign.center,
              ...
              ElevatedButton(
                onPressed: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const LogFileViewerPage(),
                    ),
                  );
                },
                child: Column(children: [
                  Text(localizations.view_log_file),
                ]),
              ),
              const SizedBox(height: 10),
              ElevatedButton(
                onPressed: FileLog.path == null
                    ? null
                    : () async {
                        await Clipboard.setData(
                            ClipboardData(text: FileLog.path!));
                      },
                child: Column(children: [
                  Text(localizations.copy_log_file_path),
                ]),
              ),
              const SizedBox(height: 10),
              ElevatedButton(
                onPressed: () async {
                  await Clipboard.setData(
                      ClipboardData(text: await FileLog.read()));
                },
                child: Column(children: [
                  Text(localizations.copy_log_file_to_clipboard),
                ]),
              ),
              const SizedBox(height: 10),
              ElevatedButton(
                onPressed: () async {
                  // The whole file is exported, not the truncated tail shown in
                  // the viewer: this is the copy that goes into a bug report.
                  final bytes = Uint8List.fromList(
                      utf8.encode(await FileLog.read(maxBytes: 1 << 30)));
                  final name =
                      "chameleon-log${DateTime.now().toIso8601String().replaceAll(':', '-')}";
                ...
                child: Column(children: [
                  Text(localizations.export_log_file),
                ]),
              ),
              const SizedBox(height: 10),
              ElevatedButton(
                ...
                child: Column(children: [
                  Text(localizations.clear_log_file),
                ]),
              ),
              const SizedBox(height: 20),
